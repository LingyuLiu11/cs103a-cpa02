...
app.get("/booklist/expensive", async (req, res, next) => {
  try {
    const books = await Book.find({price: 50.99});
    res.locals.books = books;
    res.render("booklist");
  } catch (e) {
    next(e);
...
app.get("/booklist/:auth", async (req, res, next) => {
  try {
    const { auth } = req.params;
    const books = await Book.find({author: auth});
    res.locals.books = books;
    res.render("booklist");
  } catch (e) {
    next(e);
